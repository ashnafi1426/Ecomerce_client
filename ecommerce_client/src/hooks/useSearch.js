...
export function useSearch(options = {}) {
  ...
  // Create a serialized version of filters to force re-render on any change
   ...
   * CRITICAL FIX: Use filtersKey (serialized filters) to force re-render
   * This ensures search triggers whenever ANY filter value changes
   */
  const performSearch = useCallback(async () => {
    try {
      ...
      // Build query parameters directly from filters object
      const filterParams = {};
      ...
      if (filters.brands && filters.brands.length > 0) {
        filterParams.brands = filters.brands.join(',');
      }
  ...
  }, [debouncedQuery, filtersKey, sortBy, page, limit, filters]);
